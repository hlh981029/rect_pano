...
GlobalWarping::GlobalWarping(cv::Mat& _image, cv::Mat& _mask, cv::Point** mesh, int _meshRows, int _meshCols)
...
void GlobalWarping::calcMeshToVertex()
{
...
void GlobalWarping::calcMeshShapeEnergy()
{
...
void GlobalWarping::calcBoundaryEnergy()
{
...
void GlobalWarping::calcMeshLineEnergy()
{
...
void GlobalWarping::detectLineSegment()
{
    cv::Mat grayImage;
    int lineNumber;
    double* line;
...
void GlobalWarping::cutLineSegment(vector<LineSegment>& lineSegments)
{
...
bool GlobalWarping::inQuad(Coordinate point, Coordinate topLeft, Coordinate topRight, Coordinate bottomLeft, Coordinate bottomRight)
{
...
vector<Coordinate> GlobalWarping::getIntersectionWithQuad(LineSegment line, Coordinate topLeft, Coordinate topRight, Coordinate bottomLeft, Coordinate bottomRight)
{
...
bool GlobalWarping::lineIntersectLine(LineSegment line, double slope, double intersect, bool vertical, Coordinate& intersectPoint)
{
...
BilinearWeight GlobalWarping::getBilinearWeight(Coordinate point, Coordinate topLeft, Coordinate topRight, Coordinate bottomLeft, Coordinate bottomRight)
{
    /*
    *
...
void GlobalWarping::calcBilinearWeight()
{
    Coordinate topLeft, topRight, bottomLeft, bottomRight, linePoint1, linePoint2;
...
void GlobalWarping::calcRadianBin()
{
...
void GlobalWarping::drawMesh(Coordinate** mesh)
{
    cv::Mat tempImage;
    image.copyTo(tempImage);
...
#endif // DRAW_LINE
    //cv::imwrite(imageName + "_line.jpg", tempImage);
    for (int i = 0; i < meshRows; i++) {
        for (int j = 0; j < meshCols; j++) {
    ...
    //cv::imwrite(imageName + "_mesh_line.jpg", tempImage);
    cv::imshow("mesh", tempImage);
    cv::waitKey(0);
}

void GlobalWarping::calcCost(Coordinate** mesh)
{
...
void GlobalWarping::calcLineCost(Coordinate** mesh)
{
    MatrixXd lineCost;
    VectorXd y(2 * (meshRows + 1) * (meshCols + 1));
    for (int i = 0; i <= meshRows; i++) {
...
void GlobalWarping::updateTheta()
{
    double radian, delta;
    VectorXd quadVector = VectorXd::Zero(8), lineVector;
...
void GlobalWarping::test(Coordinate** mesh, string str)
{
...
void GlobalWarping::stretch()
{
    double xMax, xMin, yMax, yMin, xHatMax, xHatMin, yHatMax, yHatMin;
    Coordinate topLeft, topRight, bottomLeft, bottomRight, linePoint1, linePoint2;
    for (int i = 0; i < meshRows; i++) {
        for (int j = 0; j < meshCols; j++) {
            xHatMax = max(meshVertex[i][j].col, max(meshVertex[i][j + 1].col, max(meshVertex[i + 1][j].col, meshVertex[i + 1][j + 1].col)));
            xHatMin = min(meshVertex[i][j].col, min(meshVertex[i][j + 1].col, min(meshVertex[i + 1][j].col, meshVertex[i + 1][j + 1].col)));
            yHatMax = max(meshVertex[i][j].row, max(meshVertex[i][j + 1].row, max(meshVertex[i + 1][j].row, meshVertex[i + 1][j + 1].row)));
            yHatMin = min(meshVertex[i][j].row, min(meshVertex[i][j + 1].row, min(meshVertex[i + 1][j].row, meshVertex[i + 1][j + 1].row)));
            xMax = max(newMeshVertex[i][j].col, max(newMeshVertex[i][j + 1].col, max(newMeshVertex[i + 1][j].col, newMeshVertex[i + 1][j + 1].col)));
            xMin = min(newMeshVertex[i][j].col, min(newMeshVertex[i][j + 1].col, min(newMeshVertex[i + 1][j].col, newMeshVertex[i + 1][j + 1].col)));
            yMax = max(newMeshVertex[i][j].row, max(newMeshVertex[i][j + 1].row, max(newMeshVertex[i + 1][j].row, newMeshVertex[i + 1][j + 1].row)));
            yMin = min(newMeshVertex[i][j].row, min(newMeshVertex[i][j + 1].row, min(newMeshVertex[i + 1][j].row, newMeshVertex[i + 1][j + 1].row)));
            scaleX += ((xMax - xMin) / (xHatMax - xHatMin));
            scaleY += ((yMax - yMin) / (yHatMax - yHatMin));
        }
    }
    scaleX /= (double)(meshRows * meshCols);
    scaleY /= (double)(meshRows * meshCols);
//
//    double oldCols = cols, oldRows = rows;
//    cv::Mat newImage, newMask;
//    cv::resize(image, newImage, cv::Size(), 1.0/scaleX, 1.0/scaleY);
//    cv::resize(mask, newMask, cv::Size(), 1.0/scaleX, 1.0/scaleY);
//    image = newImage;
//    mask = newMask;
//    cols = image.cols;
//    rows = image.rows;
//    for (int i = 0; i <= meshRows; i++) {
//        for (int j = 0; j <= meshCols; j++) {
//            meshVertex[i][j].col = meshVertex[i][j].col * oldCols / cols;
//            meshVertex[i][j].row = meshVertex[i][j].row * oldRows / rows;
//        }
//    }
//    meshLineSegment = new vector<LineSegment> * [meshRows];
//    meshLineBilinearWeight = new vector<MatrixXd> * [meshRows];
...
//    detectLineSegment();
//    drawMesh(meshVertex);
//    //calcCost(gw.meshVertex);
//    calcMeshLineEnergy();
//    for (int i = 0; i < 10; i++) {
//        cout << "iter: " << i + 1 << endl;
//        updateV();
//        drawMesh(newMeshVertex);
//        //calcCost(gw.newMeshVertex);
//        updateTheta();
//        calcMeshLineEnergy();
//        //calcLineCost(gw.newMeshVertex);
//    }
}
