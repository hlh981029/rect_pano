...
struct Coordinate {
    ...
    const cv::Point toPoint(double ratio) {
        return cv::Point(col / ratio, row / ratio);
    }
...
{
public:
    GlobalWarping(cv::Mat& _image, cv::Mat& _mask, cv::Point** mesh, int _meshRows, int _meshCols);
